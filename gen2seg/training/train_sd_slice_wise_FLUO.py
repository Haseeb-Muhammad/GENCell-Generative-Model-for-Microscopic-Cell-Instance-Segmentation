...
def main():
    ...
    #Initializing Modified U-Net
    pretrained_unet = UNet2DConditionModel.from_pretrained(args.pretrained_model_name_or_path, subfolder="unet", revision=None)
    unet_config = pretrained_unet.config.copy()
    del pretrained_unet  # Free memory
    # Modify the input channels in the config
    unet_config['in_channels'] = 12
    # Create new UNet with modified config (random weights)
    ...
    train_dataset_fluo = Fluo_N3DH_SIM(root_dir=fluo_root_dir, split="train")
    val_dataset_fluo = Fluo_N3DH_SIM(root_dir=fluo_root_dir, split="val")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(tqdm(train_dataloader, total=len(train_dataloader), desc="training")):
            with accelerator.accumulate(unet):
                ...
                # print(f"{batch['rgb'].shape=}") # 
                # Encode RGB to latents

                batch_img = batch["rgb"].to(device=accelerator.device, dtype=weight_dtype).transpose(0,1) # C, B, H, W
                # print(f"{batch_img.shape=}")

                rgb_latents = []
                for i in range(3):
                    curr_slice = batch_img[i] # B, H, W
                    # print(f"{curr_slice.shape=}")
                    curr_slice = curr_slice.unsqueeze(1).repeat(1,3,1,1) # 3*B, H, W 
                    # print(f"{curr_slice.shape=}")
                    slice_latent = encode_image(vae,curr_slice.to(device=accelerator.device, dtype=weight_dtype)) 
                    # print(f"{rgb_latents.shape=}") # ([2, 4, 60, 80]
                    rgb_latents.append(slice_latent)
                rgb_latents = torch.cat(rgb_latents, dim=1)                
                # print(f"{rgb_latents.shape=}")

                # rgb_latents = encode_image(
                #     vae,
                #     batch["rgb"].to(device=accelerator.device, dtype=weight_dtype) # B, C, H, W
                # )
                ...
                noisy_latents = torch.zeros_like(slice_latent)
                ...
                # print(f"{beta_product.shape=}")
                # print(f"{noisy_latents.shape=}")
                # print(f"{model_prediction.shape=}")
