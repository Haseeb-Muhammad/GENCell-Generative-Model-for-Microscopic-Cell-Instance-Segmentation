...
from util.semantic_loss import InstanceSegmentationLoss
...
def main():
    ...
    def rgb_to_binary(img, threshold=1):
        '''
    ...
    def calculate_IoU(pred, gt): 
        ...
        binary_pred = rgb_to_binary(np.array(pred),threshold=1)
        binary_gt = rgb_to_binary(np.array(gt), threshold=1)
    ...
    def validation(epoch, global_step):
        ...
        with torch.no_grad(): 
            for batch_index, batch in enumerate(tqdm(val_dataloader, total=len(val_dataloader), desc="Validating")):
                ...
                estimation_loss = instance_loss(seg.to("cuda"), ground_truth.to("cuda"), batch["no_bg"])
            ...
            accelerator.log({"validation_loss" : mean_val_loss,
                              ...
                              "val updated intra loss" : instance_loss.updated_intra_instance/len(val_dataloader),
                              "val updated inter loss" : instance_loss.updated_inter_instance/len(val_dataloader),
                            })
            
            instance_loss.updated_intra_instance = 0
            instance_loss.updated_inter_instance = 0
    ...
    if accelerator.is_main_process:
        ...
        wandb_tracker.define_metric(name="val updated intra loss", step_metric="global_step")
        wandb_tracker.define_metric(name="val_updated inter loss", step_metric="global_step")
        wandb_tracker.define_metric(name="IoU", step_metric="global_step")

        wandb_tracker.define_metric(name="updated intra loss",step_metric="global_step")
        wandb_tracker.define_metric(name="updated inter loss",step_metric="global_step")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
      ...
      #  print("IMPORTANT: NORM DISABLED")
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            instance_loss.updated_intra_instance = instance_loss.updated_intra_instance / args.gradient_accumulation_steps 
            instance_loss.updated_inter_instance = instance_loss.updated_inter_instance / args.gradient_accumulation_steps
            ...
            if accelerator.sync_gradients:
                ...
                accelerator.log(
                    {
                        "updated intra loss" : instance_loss.updated_intra_instance,
                        "updated inter loss" : instance_loss.updated_inter_instance,
