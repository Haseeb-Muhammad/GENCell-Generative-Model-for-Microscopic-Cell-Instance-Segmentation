...
class Fluo_N3DH_SIM_with_neighbors(Dataset):
    def __init__(self, root_dir, split, transform=True, height=480, width=640):
        self.root_dir = root_dir
        self.split = split
        self.image_paths, self.gt_paths = self._loadPaths()
        if transform == True:
            self.transform = Fluo_N3DH_SIM_transform(H=height, W=width)
        self.neighbor_info = self._load_neighbors()
    
    def _load_neighbors(self,):
        with open(os.path.join(self.root_dir, "gt", f"{self.split}_neighbors.json"), "r", encoding="utf-8") as f:
