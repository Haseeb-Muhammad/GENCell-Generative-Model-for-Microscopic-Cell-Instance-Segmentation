...
from util.cross_entropy_loss import BinaryCrossEntropy
...
def main():
    ...
    instance_loss = BinaryCrossEntropy()
    res = (640, 648)
    ...
    def calculate_IoU(pred, gt): 
        ...
        # print(f"{pred.shape=}")
        # print(f"{gt.shape=}")
        binary_pred = rgb_to_binary(np.array(pred), threshold=127)
    ...
    def validation(epoch, global_step):
        ...
        with torch.no_grad(): 
            for batch_index, batch in enumerate(tqdm(val_dataloader, total=len(val_dataloader), desc="Validating")):
                ...
                estimation_loss = instance_loss(seg.to("cuda"), ground_truth.to("cuda"))
            ...
            accelerator.log({"validation_loss" : mean_val_loss, "IoU" : IoU,"global_step":global_step})
      ...
      #  print("IMPORTANT: NORM DISABLED")
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                estimation_loss = instance_loss(current_estimate, ground_truth)
